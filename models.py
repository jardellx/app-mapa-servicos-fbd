...
# CORREÇÃO: A única importação local necessária é a 'Base' da conexão.
from db_connection import Base 

#
# NENHUMA OUTRA IMPORTAÇÃO (panel, pandas, backend_operations) É NECESSÁRIA AQUI.
#
...
class Usuario_comum(Base):
    ...
    loc_atual_coord = Column(String(100), nullable=True)
    usuario_base = relationship("Usuario", back_populates="usuario_comum_rel") 

class Adm(Base):
    __tablename__ = 'adm'
    id_usuario = Column(Integer, ForeignKey('usuario.id_usuario'), primary_key=True)
    nivel_acesso = Column(String(100), nullable=True)
